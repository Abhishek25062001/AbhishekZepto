...
export function LoginPage() {
  ...
  const handleLogin = handleSubmit(async values => {
    ...
    try {
      ...
      logAdminAuthEvent('request_otp_success', {
      ...
      logAdminAuthEvent('request_otp_failure', {
    ...
    <AuthLayout>
      <Card title="Admin Login">
        <form onSubmit={handleLogin} style={{ display: 'grid', gap: 'var(--spacing-lg)', width: '320px' }}>
          ...
          <Button aria-label="Send admin login OTP" loading={isSubmitting} type="submit">Continue</Button>
        </form>
      </Card>
    </AuthLayout>
  );
