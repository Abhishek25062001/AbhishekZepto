...
import { saveAdminSession } from '../../services/auth/session-storage.service';
import { useAuthStore } from '../../store/auth.store';
import { logAdminAuthEvent } from '../../utils/auth-event-logger';
import { getFieldError } from '../../utils/form-error.util';
import { isAdminAuthResponse, mapVerifyOtpResponseToAdminSession } from '../../utils/auth-response.util';
import {
  ...
  const handleVerify = () => {
    void handleSubmit(async (values) => {
      ...
      try {
        ...
        if (!isAdminAuthResponse(response.data)) {
          setScreenError('This account cannot be used in the Admin Dashboard.');
          return;
        }

        const adminSession = mapVerifyOtpResponseToAdminSession(response.data);

        saveAdminSession(adminSession);
        setAuthSession(adminSession);
        logAdminAuthEvent('verify_otp_success', {
        ...
        logAdminAuthEvent('verify_otp_failure', {
          challengeId,
    ...
    <AuthLayout>
      <Card description={`Enter the code sent to ${state.maskedTarget}.`} title="Verify OTP">
        <div style={{ display: 'grid', gap: 'var(--spacing-lg)', width: '320px' }}>
          ...
          <Input
            aria-label="Admin one-time password code"
          ...
          />
          <Button
            aria-label="Verify admin OTP and sign in"
          ...
          <Button
            aria-label="Resend admin one-time password"
          ...
          <Button
            aria-label="Change admin phone number"
            onClick={() => {
              setValue('otp', '');
              navigate('/login', { replace: true });
            }}
