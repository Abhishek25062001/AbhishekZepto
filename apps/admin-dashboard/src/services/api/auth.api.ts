...
export type AdminPermissionsResponse = AuthUserResponse & {
  adminId: string;
};
...
export const getAdminPermissions = async (): Promise<
  ApiSuccessResponse<AdminPermissionsResponse>
> => {
  const response = await apiClient.get<ApiSuccessResponse<AdminPermissionsResponse>>(
