...
export type VendorPermissionsResponse = AuthUserResponse & {
  vendorUserId: string;
};
...
export const getVendorPermissions = async (): Promise<
  ApiSuccessResponse<VendorPermissionsResponse>
> => {
  const response = await apiClient.get<ApiSuccessResponse<VendorPermissionsResponse>>(
    '/api/v1/vendor/me/permissions',
  );
