...
export type CustomerPermissionsResponse = AuthUserResponse & {
  customerId: string;
...
export const getCustomerPermissions = async (): Promise<
  ApiSuccessResponse<CustomerPermissionsResponse>
> => {
  const response = await apiClient.get<ApiSuccessResponse<CustomerPermissionsResponse>>(
    '/api/v1/customer/me/permissions',
  );
