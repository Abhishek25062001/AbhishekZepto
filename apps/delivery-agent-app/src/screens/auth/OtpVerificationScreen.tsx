...
import { saveDeliverySession } from '../../services/auth/session-storage.service';
...
import {
  ...
  mapVerifyOtpResponseToDeliverySession,
...
export function OtpVerificationScreen({ navigation, route }: Props) {
  ...
  const handleVerify = () => {
    void handleSubmit(async (values) => {
      ...
      try {
        ...
        if (!isDeliveryAuthResponse(response.data)) {
          setScreenError('This account cannot be used in the Delivery Agent App.');
          return;
        }

        const deliverySession =
          mapVerifyOtpResponseToDeliverySession(response.data);

        await saveDeliverySession(deliverySession);
        setAuthSession(deliverySession);
        logDeliveryAuthEvent('verify_otp_success', {
          deliveryAgentId: deliverySession.deliveryAgentId,
          role: deliverySession.role,
        });
      } catch (error) {
        const apiError = error as { response?: { data?: ApiErrorResponse } };
        logDeliveryAuthEvent('verify_otp_failure', {
          challengeId,
  ...
  return (
    <ScreenWrapper>
      ...
      <Input
        accessibilityLabel="Delivery agent one-time password code"
