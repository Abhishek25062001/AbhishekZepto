...
const VENDOR_STORE_STATUSES = new Set<VendorOrderStoreStatus>([
  'pending_acceptance',
  'accepted',
  'rejected',
]);

const ORDER_EVENT_NAMES = new Set<VendorRealtimeEventName>([
  VENDOR_REALTIME_EVENTS.ORDER_CREATED,
  VENDOR_REALTIME_EVENTS.ORDER_STATUS_UPDATED,
  VENDOR_REALTIME_EVENTS.ORDER_CANCELLED,
]);

const PICKUP_EVENT_NAMES = new Set<VendorRealtimeEventName>([
...
const isVendorRealtimeEventName = (
  value: unknown,
): value is VendorRealtimeEventName =>
  typeof value === 'string' &&
  Object.values(VENDOR_REALTIME_EVENTS).includes(
    value as VendorRealtimeEventName,
  );
...
const normalizeOrderStatus = (
  ...
  if (rawStatus && VENDOR_ORDER_STATUSES.has(rawStatus as VendorOrderStatus)) {
    return rawStatus as VendorOrderStatus;
  }

  if (eventName === VENDOR_REALTIME_EVENTS.ORDER_CANCELLED) {
    return 'cancelled';
  }
...
const normalizeStoreStatus = (
  ...
  if (rawStatus && VENDOR_STORE_STATUSES.has(rawStatus as VendorOrderStoreStatus)) {
    return rawStatus as VendorOrderStoreStatus;
  }

  return orderStatus === 'placed' ? 'pending_acceptance' : 'accepted';
};

const buildVendorOrderListItem = (
  data: Record<string, unknown>,
  orderId: string,
  storeId: string,
  orderStatus: VendorOrderStatus,
...
export const mapVendorRealtimeEventPayload = (
  payload: VendorRealtimeSocketPayload,
  fallbackEventName: VendorRealtimeEventName,
): VendorRealtimeEvent | null => {
  ...
  if (ORDER_EVENT_NAMES.has(eventName)) {
    ...
    const orderEvent: VendorOrderRealtimeEvent = {
      eventName: eventName as VendorOrderRealtimeEvent['eventName'],
      orderId,
      storeId,
      orderStatus,
      totalAmount,
      ...
      order: buildVendorOrderListItem(
        data,
        orderId,
        storeId,
        orderStatus,
  ...
  if (PICKUP_EVENT_NAMES.has(eventName)) {
    ...
    const riderId =
      toStringValue(data.riderId) ?? toStringValue(data.deliveryAgentId);
    const pickupStatus =
    ...
    const pickupEvent: VendorPickupRealtimeEvent = {
      eventName: eventName as VendorPickupRealtimeEvent['eventName'],
      orderId,
      assignmentId,
      riderId,
      pickupStatus,
