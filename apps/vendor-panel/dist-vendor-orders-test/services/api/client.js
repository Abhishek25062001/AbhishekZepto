...
exports.apiClient.interceptors.request.use((config) => {
    ...
    config.headers['x-trace-id'] = config.headers?.['x-trace-id'] ?? 'vendor-web-request';
    if (accessToken) {
        config.headers.Authorization = `Bearer ${accessToken}`;
    }
    if (env_1.isDevelopment) {
        console.debug('Vendor Panel API request', {
...
exports.apiClient.interceptors.response.use((response) => {
    if (env_1.isDevelopment) {
        console.debug('Vendor Panel API response', {
    ...
    if (env_1.isDevelopment) {
        console.debug('Vendor Panel API response error', {
