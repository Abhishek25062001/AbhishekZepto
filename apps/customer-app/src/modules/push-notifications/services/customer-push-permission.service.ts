...
const requestAndroidNotificationPermission = async (): Promise<CustomerPushPermissionStatus> => {
...
export const getPushPermissionStatus = async (): Promise<CustomerPushPermissionStatus> => {
  if (Platform.OS === 'android') {
    if (Platform.Version < 33) {
      return 'granted';
    }
...
export const requestPushPermission = async (): Promise<CustomerPushPermissionStatus> => {
  if (Platform.OS === 'android') {
