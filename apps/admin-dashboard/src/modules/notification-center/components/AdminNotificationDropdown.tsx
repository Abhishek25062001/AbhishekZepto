...
export const AdminNotificationDropdown = () => {
  ...
  const loadPreview = async () => {
    ...
    try {
      const [listResult, unreadResult] = await Promise.all([
        adminNotificationApi.list({ limit: 5 }),
  ...
  const openNotification = async (notification: InAppNotification) => {
    ...
    try {
      await adminNotificationApi.markRead(notification.id);
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      markAllRead();
      const unreadResult = await adminNotificationApi.unreadCount();
      setUnreadCount(unreadResult.unreadCount);
    } finally {
          ...
          {isLoading ? (
            ...
            <ul>
              {notifications.map((notification) => (
                <li key={notification.id} data-priority={notification.priority} data-read={notification.isRead}>
